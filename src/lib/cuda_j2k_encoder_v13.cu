/*
    CUDA JPEG2000 Encoder v13 — Fused Kernels

    V13: Fuse int-to-float conversion with the first horizontal DWT pass.
    Instead of separate i2f + dwt_h kernels (2 global memory round-trips),
    we read int32 directly, convert in shared memory, and do the DWT lifting
    all in one kernel launch. Saves one full memory pass per component.

    Also fuses quantization with the final download to avoid an extra
    device-to-device copy.

    Based on v12's generic architecture with auto-tuning.
*/
...
/*
 * Fused int32->float + horizontal DWT.
 * Reads int32 input, converts to float in shared memory,
 * performs all 4 lifting steps, writes float output.
 * Saves one full global memory round-trip vs separate i2f + dwt_h.
 */
__global__ void v13_fused_i2f_dwt_h(const int32_t* __restrict__ input,
                                      ...
                                      int w, int h, int s)
{
    ...
    /* Load int32 and convert to float directly into shared memory */
    for (int i = t; i < w; i += nt)
        sm[i] = __int2float_rn(input[y * s + i]);
    __syncthreads();

    /* Lifting step 1: alpha on odd */
...
/* Standard horizontal DWT for levels 1-4 (input is already float) */
__global__ void v13_dwt_h(float* __restrict__ data,
                           float* __restrict__ out,
                           int w, int h, int s)
{
    extern __shared__ float sm[];
...
/* Vertical DWT (same as v12) */
__global__ void v13_dwt_v(float* __restrict__ data,
                           float* __restrict__ out,
                           int w, int h, int s)
{
...
/* Quantize + encode */
__global__ void v13_qe(const float* __restrict__ src,
...
struct CudaJ2KEncoderImpl {
    /* Device: 2 float buffers + encoded output per component.
     * No int32 device buffer needed — fused kernel reads from host-uploaded int32
     * directly into the first DWT pass. BUT we still need the int32 on device
     * for the fused kernel to read from. */
    char* d_pool = nullptr;
    float* d_c[3], *d_t[3];
    int32_t* d_in[3];
    uint8_t* d_enc[3];
    ...
    bool ensure(int w, int h, size_t epc) {
        ...
        if (cudaHostAlloc(&h_pool, 3 * n * sizeof(int32_t) + 3 * epc,
                        ...
                        int64_t bit_rate, int fps, bool is_3d, bool is_4k)
{
    ...
    for (int c = 0; c < 3; ++c) {
        ...
        /* Level 0: FUSED int32->float + horizontal DWT (saves one memory pass) */
        size_t smem = w * sizeof(float);
        v13_fused_i2f_dwt_h<<<h, h_blk, smem, _impl->st[c]>>>(
            _impl->d_in[c], _impl->d_c[c], w, h, stride);
        A = _impl->d_c[c];

        /* Level 0: vertical DWT */
        v13_dwt_v<<<(w + v_blk - 1) / v_blk, v_blk, 0, _impl->st[c]>>>(
            A, B_, w, h, stride);
        float* t = A; A = B_; B_ = t;
        w = (w + 1) / 2;
        h = (h + 1) / 2;

        /* Levels 1-4: standard float->float DWT */
        for (int l = 1; l < NUM_DWT_LEVELS; ++l) {
            smem = w * sizeof(float);
            v13_dwt_h<<<h, h_blk, smem, _impl->st[c]>>>(A, B_, w, h, stride);
            t = A; A = B_; B_ = t;
        ...
        /* Quantize + download */
        v13_qe<<<(tpc + gen_blk - 1) / gen_blk, gen_blk, 0, _impl->st[c]>>>(
