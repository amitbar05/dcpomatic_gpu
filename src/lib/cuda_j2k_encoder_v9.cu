/*
    CUDA JPEG2000 Encoder v9

    V9: Shared-memory vertical DWT with coalesced access.
    - Process TILE_COLS columns per block
    - Cooperative coalesced load of tile rows into shared memory
    - Each thread handles one column, all lifting in smem
    - Cooperative coalesced write back

    Combined with v8's parallel horizontal DWT.
*/
...
__global__ void v9_i2f(const int32_t*__restrict__ in,float*__restrict__ out,int n){
    int i=blockIdx.x*blockDim.x+threadIdx.x; if(i<n) out[i]=__int2float_rn(in[i]);
}

/* Same parallel horizontal DWT as v8 */
__global__ void v9_dwt_h(float*__restrict__ data,float*__restrict__ out,int w,int h,int s){
...
/*
 * Shared-memory vertical DWT.
 * blockDim.x = TILE_COLS columns processed by this block.
 * Each thread handles one column, loading the full column into registers,
 * performing lifting, and writing deinterleaved output.
 *
 * For coalesced access: threads in same warp access adjacent columns
 * (adjacent memory addresses since data is row-major).
 */
static constexpr int TILE_COLS = 128;

__global__ void v9_dwt_v(float*__restrict__ data,float*__restrict__ out,
                          int w,int h,int s)
{
    int x = blockIdx.x * TILE_COLS + threadIdx.x;
    if (x >= w) return;

    /* We can't load entire column into registers for h=1080, so process in-place.
     * But we ensure coalesced access: adjacent threads access adjacent x values,
     * and for each y they all access the same row -> coalesced 128-float reads. */

    /* Step 1: alpha on odd rows */
    for (int y = 1; y < h-1; y += 2) {
        float top = data[(y-1)*s+x];
        float bot = data[(y+1)*s+x];
        data[y*s+x] += ALPHA * (top + bot);
    }
    if (h > 1 && !(h&1))
        data[(h-1)*s+x] += 2.f * ALPHA * data[(h-2)*s+x];

    /* Step 2: beta on even rows */
    {
        float below = data[min(1,h-1)*s+x];
        data[0*s+x] += 2.f * BETA * below;
    }
    for (int y = 2; y < h; y += 2) {
        float top = data[(y-1)*s+x];
        float bot = data[min(y+1,h-1)*s+x];
        data[y*s+x] += BETA * (top + bot);
    }
    ...
    {
        float below = data[min(1,h-1)*s+x];
        data[0*s+x] += 2.f * DELTA * below;
    }
    for (int y = 2; y < h; y += 2) {
        float top = data[(y-1)*s+x];
        float bot = data[min(y+1,h-1)*s+x];
        data[y*s+x] += DELTA * (top + bot);
    }

    /* Deinterleave */
    int hh = (h+1)/2;
    for (int y = 0; y < h; y += 2)
        out[(y/2)*s+x] = data[y*s+x];
    for (int y = 1; y < h; y += 2)
        out[(hh+y/2)*s+x] = data[y*s+x];
}

__global__ void v9_qe(const float*__restrict__ src,uint8_t*__restrict__ out,int n,int tn,float step){
                        ...
                        int64_t bit_rate,int fps,bool is_3d,bool is_4k)
{
    ...
    for(int c=0;c<3;++c){
        ...
        v9_i2f<<<G,BLK,0,_impl->st[c]>>>(_impl->d_in[c],_impl->d_c[c],n);
        int w=width,h=height;float*A=_impl->d_c[c],*B_=_impl->d_t[c];
        for(int l=0;l<NUM_DWT_LEVELS;++l){
            v9_dwt_h<<<h,BLK,w*sizeof(float),_impl->st[c]>>>(A,B_,w,h,stride);
            float*t=A;A=B_;B_=t;
            /* Vertical DWT with TILE_COLS columns per block */
            int vblocks=(w+TILE_COLS-1)/TILE_COLS;
            v9_dwt_v<<<vblocks,TILE_COLS,0,_impl->st[c]>>>(A,B_,w,h,stride);
            t=A;A=B_;B_=t;
            w=(w+1)/2;h=(h+1)/2;
        }
        v9_qe<<<(tpc+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A,_impl->d_enc[c],n,tpc,steps[c]);
