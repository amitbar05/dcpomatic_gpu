/*
    CUDA JPEG2000 Encoder v6

    Improvements over v5:
    - Eliminated final D2D memcpy (5 DWT levels x 2 passes = 10 swaps = even = no copy needed)
    - DWT operates only on the active subband region at each level
    - Reduced memcpy sizes per DWT level (only copy what's needed)
    - Quantize+encode only reads the actually-needed subband area
*/
...
__global__ void v6_i2f(const int32_t*__restrict__ in,float*__restrict__ out,int n){
    int i=blockIdx.x*blockDim.x+threadIdx.x; if(i<n) out[i]=__int2float_rn(in[i]);
}

__global__ void v6_dwt_h(float*__restrict__ data,float*__restrict__ out,int w,int h,int s){
...
__global__ void v6_dwt_v(float*__restrict__ data,float*__restrict__ out,int w,int h,int s){
...
__global__ void v6_qe(const float*__restrict__ src,uint8_t*__restrict__ out,int n,int tn,float step){
                        ...
                        int64_t bit_rate,int fps,bool is_3d,bool is_4k)
{
    ...
    for(int c=0;c<3;++c){
        ...
        v6_i2f<<<G,BLK,0,_impl->st[c]>>>(_impl->d_in[c],_impl->d_c[c],n);

        int w=width,h=height;
        float*A=_impl->d_c[c],*B_=_impl->d_t[c];
        for(int l=0;l<NUM_DWT_LEVELS;++l){
            v6_dwt_h<<<(h+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A,B_,w,h,stride);
            /* swap A<->B */
            float*t=A;A=B_;B_=t;
            v6_dwt_v<<<(w+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A,B_,w,h,stride);
            t=A;A=B_;B_=t;
            w=(w+1)/2;h=(h+1)/2;
        }
        /* After 5 levels x 2 passes = 10 swaps, A == d_c[c] (even count).
         * No final D2D copy needed! */

        v6_qe<<<(tpc+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A,_impl->d_enc[c],n,tpc,steps[c]);
