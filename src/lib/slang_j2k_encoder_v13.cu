/*
    Slang JPEG2000 Encoder v13 — Fused Kernels

    V13: Matches CUDA v13 fused kernel optimization.
    Fuses int-to-float conversion with first DWT horizontal pass,
    eliminating one global memory round-trip per component.
*/
...
__global__ void s13_fused_i2f_dwt_h(const int32_t* __restrict__ input,
...
__global__ void s13_dwt_h(float* __restrict__ data, float* __restrict__ out,
                           int w, int h, int s)
{
    extern __shared__ float sm[];
    int y = blockIdx.x;
...
__global__ void s13_dwt_v(float* __restrict__ data, float* __restrict__ out,
                           int w, int h, int s)
{
...
__global__ void s13_qe(const float* __restrict__ src, uint8_t* __restrict__ out,
                         ...
                         int64_t bit_rate, int fps, bool is_3d, bool is_4k)
{
    ...
    for (int c = 0; c < 3; ++c) {
        ...
        s13_fused_i2f_dwt_h<<<h, h_blk, w * sizeof(float), _impl->st[c]>>>(
            _impl->d_in[c], _impl->d_c[c], w, h, stride);
        A = _impl->d_c[c];

        s13_dwt_v<<<(w + v_blk - 1) / v_blk, v_blk, 0, _impl->st[c]>>>(A, B_, w, h, stride);
        float* t = A; A = B_; B_ = t;
        w = (w + 1) / 2; h = (h + 1) / 2;

        for (int l = 1; l < NUM_DWT_LEVELS; ++l) {
            s13_dwt_h<<<h, h_blk, w * sizeof(float), _impl->st[c]>>>(A, B_, w, h, stride);
            t = A; A = B_; B_ = t;
            s13_dwt_v<<<(w + v_blk - 1) / v_blk, v_blk, 0, _impl->st[c]>>>(A, B_, w, h, stride);
            t = A; A = B_; B_ = t;
            w = (w + 1) / 2; h = (h + 1) / 2;
        ...
        s13_qe<<<(tpc + gen_blk - 1) / gen_blk, gen_blk, 0, _impl->st[c]>>>(
