...
__global__ void s9_i2f(const int32_t*__restrict__ in,float*__restrict__ out,int n){
    int i=blockIdx.x*blockDim.x+threadIdx.x; if(i<n) out[i]=__int2float_rn(in[i]);
}

/* Same parallel horizontal DWT as v8 */
__global__ void s9_dwt_h(float*__restrict__ data,float*__restrict__ out,int w,int h,int s){
...
__global__ void s9_dwt_v(float*__restrict__ data,float*__restrict__ out,
                          int w,int h,int s)
{
...
__global__ void s9_qe(const float*__restrict__ src,uint8_t*__restrict__ out,int n,int tn,float step){
                        ...
                        int64_t bit_rate,int fps,bool is_3d,bool is_4k)
{
    ...
    for(int c=0;c<3;++c){
        ...
        s9_i2f<<<G,BLK,0,_impl->st[c]>>>(_impl->d_in[c],_impl->d_c[c],n);
        int w=width,h=height;float*A=_impl->d_c[c],*B_=_impl->d_t[c];
        for(int l=0;l<NUM_DWT_LEVELS;++l){
            s9_dwt_h<<<h,BLK,w*sizeof(float),_impl->st[c]>>>(A,B_,w,h,stride);
            float*t=A;A=B_;B_=t;
            /* Vertical DWT with TILE_COLS columns per block */
            int vblocks=(w+TILE_COLS-1)/TILE_COLS;
            s9_dwt_v<<<vblocks,TILE_COLS,0,_impl->st[c]>>>(A,B_,w,h,stride);
            t=A;A=B_;B_=t;
            w=(w+1)/2;h=(h+1)/2;
        }
        s9_qe<<<(tpc+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A,_impl->d_enc[c],n,tpc,steps[c]);
