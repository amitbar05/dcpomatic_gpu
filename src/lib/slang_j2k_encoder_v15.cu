...
__global__ void s15_fused_i2f_dwt_h(const int32_t* __restrict__ in, float* __restrict__ out, int w, int h, int s) {
...
__global__ void s15_dwt_h(float* __restrict__ data, float* __restrict__ out, int w, int h, int s) {
    extern __shared__ float sm[];
    int y = blockIdx.x; if (y >= h) return;
    int t = threadIdx.x, nt = blockDim.x;
...
__global__ void s15_dwt_v(float* __restrict__ data, float* __restrict__ out, int w, int h, int s) {
...
__global__ void s15_dwt_v_reg(float* __restrict__ data, float* __restrict__ out, int w, int h, int s) {
    int x = blockIdx.x*blockDim.x+threadIdx.x; if (x >= w) return;
...
__global__ void s15_qe(const float* __restrict__ src, uint8_t* __restrict__ out, int n, int tn, float step) {
                         ...
                         int64_t bit_rate,int fps,bool is_3d,bool is_4k)
{
    ...
    for(int c=0;c<3;++c){
        ...
        s15_fused_i2f_dwt_h<<<h,h_blk,w*sizeof(float),_impl->st[c]>>>(_impl->d_in[c],_impl->d_c[c],w,h,stride);
        A=_impl->d_c[c];
        s15_dwt_v<<<(w+v_blk-1)/v_blk,v_blk,0,_impl->st[c]>>>(A,B_,w,h,stride);
        float*t=A;A=B_;B_=t; w=(w+1)/2;h=(h+1)/2;

        for(int l=1;l<NUM_DWT_LEVELS;++l){
            s15_dwt_h<<<h,h_blk,w*sizeof(float),_impl->st[c]>>>(A,B_,w,h,stride);
            t=A;A=B_;B_=t;
            if(h<=MAX_REG_HEIGHT)
                s15_dwt_v_reg<<<(w+v_blk-1)/v_blk,v_blk,0,_impl->st[c]>>>(A,B_,w,h,stride);
            else
                s15_dwt_v<<<(w+v_blk-1)/v_blk,v_blk,0,_impl->st[c]>>>(A,B_,w,h,stride);
            t=A;A=B_;B_=t; w=(w+1)/2;h=(h+1)/2;
        }

        s15_qe<<<(tpc+gen_blk-1)/gen_blk,gen_blk,0,_impl->st[c]>>>(A,_impl->d_enc[c],n,tpc,steps[c]);
