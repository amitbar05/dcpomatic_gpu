...
__global__ void s10_dwt_h(float*__restrict__ data,float*__restrict__ out,int w,int h,int s){
...
/* Vertical DWT */
__global__ void s10_dwt_v(float*__restrict__ data,float*__restrict__ out,int w,int h,int s){
...
__global__ void s10_qe(const float*__restrict__ src,uint8_t*__restrict__ out,int n,int tn,float step){
                        ...
                        int64_t bit_rate,int fps,bool is_3d,bool is_4k)
{
    ...
    for(int c=0;c<3;++c){
        ...
        for(int l=0;l<NUM_DWT_LEVELS;++l){
            s10_dwt_h<<<h,BLK,w*sizeof(float),_impl->st[c]>>>(A,B_,w,h,stride);
            float*t=A;A=B_;B_=t;
            s10_dwt_v<<<(w+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A,B_,w,h,stride);
            t=A;A=B_;B_=t;
            w=(w+1)/2;h=(h+1)/2;
        }
        s10_qe<<<(tpc+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A,_impl->d_enc[c],n,tpc,steps[c]);
