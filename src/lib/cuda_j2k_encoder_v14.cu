/*
    CUDA JPEG2000 Encoder v14 — Register-Blocked Small DWT + Pipelined

    V14: Two key optimizations over v13:
    1. Register-blocked vertical DWT for small subbands (levels 2-4).
       At level 2, columns are only 270 elements, halving each level.
       For columns <= 128 elements, load entire column into registers,
       do all 4 lifting steps without global memory access, write once.
    2. Better pipelining: start uploading next component immediately,
       overlap H2D transfer with DWT compute on previous component.

    Based on v13's fused i2f+DWT-H kernel and v12's generic auto-tuning.
*/
...
/* Max column height for register-blocked vertical DWT */
...
/* Fused int32->float + horizontal DWT (from v13) */
__global__ void v14_fused_i2f_dwt_h(const int32_t* __restrict__ input,
                                      ...
                                      int w, int h, int s)
{
    ...
    for (int i = t; i < w; i += nt)
        sm[i] = __int2float_rn(input[y * s + i]);
    __syncthreads();
...
/* Standard horizontal DWT for levels 1+ */
__global__ void v14_dwt_h(float* __restrict__ data,
                           float* __restrict__ out,
                           int w, int h, int s)
{
    extern __shared__ float sm[];
...
/* Standard vertical DWT for large subbands (levels 0-1) */
__global__ void v14_dwt_v(float* __restrict__ data,
                           float* __restrict__ out,
                           int w, int h, int s)
{
 ...
 * Load the entire column (up to MAX_REG_HEIGHT elements) into a local array,
 * perform all 4 lifting steps in registers (no global memory access),
 * write the deinterleaved result once.
 *
 * This eliminates 7 global memory passes (4 lifting reads + 3 intermediate writes)
 * for the vertical DWT at small levels.
 */
__global__ void v14_dwt_v_reg(float* __restrict__ data,
                               float* __restrict__ out,
                               int w, int h, int s)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (h > 1 && !(h & 1))
        col[h - 1] += 2.f * ALPHA * col[h - 2];

    /* Step 2: beta on even */
    col[0] += 2.f * BETA * col[min(1, h - 1)];
    for (int y = 2; y < h; y += 2)
        col[y] += BETA * (col[y - 1] + col[min(y + 1, h - 1)]);

    /* Step 3: gamma on odd */
    for (int y = 1; y < h - 1; y += 2)
        col[y] += GAMMA * (col[y - 1] + col[y + 1]);
    if (h > 1 && !(h & 1))
        col[h - 1] += 2.f * GAMMA * col[h - 2];

    /* Step 4: delta on even */
    col[0] += 2.f * DELTA * col[min(1, h - 1)];
    for (int y = 2; y < h; y += 2)
        col[y] += DELTA * (col[y - 1] + col[min(y + 1, h - 1)]);

    /* Deinterleave: write once */
    int hh = (h + 1) / 2;
    for (int y = 0; y < h; y += 2)
        out[(y / 2) * s + x] = col[y];
    for (int y = 1; y < h; y += 2)
        out[(hh + y / 2) * s + x] = col[y];
}

/* Quantize + encode */
__global__ void v14_qe(const float* __restrict__ src,
                        ...
                        int64_t bit_rate, int fps, bool is_3d, bool is_4k)
{
    ...
    for (int c = 0; c < 3; ++c) {
        ...
        /* Level 0: fused i2f + DWT-H */
        v14_fused_i2f_dwt_h<<<h, h_blk, w * sizeof(float), _impl->st[c]>>>(
            _impl->d_in[c], _impl->d_c[c], w, h, stride);
        A = _impl->d_c[c];

        /* Level 0: vertical DWT (large subband, use standard kernel) */
        v14_dwt_v<<<(w + v_blk - 1) / v_blk, v_blk, 0, _impl->st[c]>>>(A, B_, w, h, stride);
        float* t = A; A = B_; B_ = t;
        w = (w + 1) / 2; h = (h + 1) / 2;

        /* Levels 1-4 */
        for (int l = 1; l < NUM_DWT_LEVELS; ++l) {
            v14_dwt_h<<<h, h_blk, w * sizeof(float), _impl->st[c]>>>(A, B_, w, h, stride);
            t = A; A = B_; B_ = t;

            /* Use register-blocked kernel for small columns */
            if (h <= MAX_REG_HEIGHT) {
                v14_dwt_v_reg<<<(w + v_blk - 1) / v_blk, v_blk, 0, _impl->st[c]>>>(
                    A, B_, w, h, stride);
            } else {
                v14_dwt_v<<<(w + v_blk - 1) / v_blk, v_blk, 0, _impl->st[c]>>>(
                    A, B_, w, h, stride);
            }
            t = A; A = B_; B_ = t;
            w = (w + 1) / 2; h = (h + 1) / 2;
        ...
        v14_qe<<<(tpc + gen_blk - 1) / gen_blk, gen_blk, 0, _impl->st[c]>>>(
    ...
    cs[psot_pos] = tile_len >> 24; cs[psot_pos+1] = (tile_len >> 16) & 0xFF;
    cs[psot_pos+2] = (tile_len >> 8) & 0xFF; cs[psot_pos+3] = tile_len & 0xFF;
    w16(J2K_EOC);
    return cs;
}
