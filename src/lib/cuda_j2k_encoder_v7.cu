...
__global__ void v7_i2f(const int32_t*__restrict__ in,float*__restrict__ out,int n){
 ...
 */
__global__ void v7_dwt_h_smem(float*__restrict__ data, float*__restrict__ out,
                               int w, int h, int s)
{
 ...
 */
__global__ void v7_dwt_h(float*__restrict__ data,float*__restrict__ out,int w,int h,int s){
...
__global__ void v7_dwt_v(float*__restrict__ data,float*__restrict__ out,int w,int h,int s){
...
__global__ void v7_qe(const float*__restrict__ src,uint8_t*__restrict__ out,int n,int tn,float step){
                        ...
                        int64_t bit_rate,int fps,bool is_3d,bool is_4k)
{
    ...
    for(int c=0;c<3;++c){
        ...
        v7_i2f<<<G,BLK,0,_impl->st[c]>>>(_impl->d_in[c],_impl->d_c[c],n);
        ...
        for(int l=0;l<NUM_DWT_LEVELS;++l){
            if (w <= MAX_ROW_W) {
                ...
                v7_dwt_h_smem<<<h, BLK, smem_size, _impl->st[c]>>>(A, B_, w, h, stride);
            } else {
                v7_dwt_h<<<(h+BLK-1)/BLK, BLK, 0, _impl->st[c]>>>(A, B_, w, h, stride);
            }
            float*t=A;A=B_;B_=t;

            v7_dwt_v<<<(w+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A,B_,w,h,stride);
            t=A;A=B_;B_=t;
            w=(w+1)/2;h=(h+1)/2;
        }
        /* 10 swaps -> A == d_c[c], no copy needed */

        v7_qe<<<(tpc+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A,_impl->d_enc[c],n,tpc,steps[c]);
