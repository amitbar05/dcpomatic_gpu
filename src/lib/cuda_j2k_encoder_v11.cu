...
__global__ void v11_i2f(const int32_t*__restrict__ in,float*__restrict__ out,int n){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i<n)out[i]=__int2float_rn(in[i]);
}

__global__ void v11_dwt_h(float*__restrict__ data,float*__restrict__ out,int w,int h,int s){
...
__global__ void v11_dwt_v(float*__restrict__ data,float*__restrict__ out,int w,int h,int s){
...
__global__ void v11_qe(const float*__restrict__ src,uint8_t*__restrict__ out,int n,int tn,float step){
                        ...
                        int64_t bit_rate,int fps,bool is_3d,bool is_4k)
{
    ...
    for(int c=0;c<3;++c){
        ...
        v11_i2f<<<G,BLK,0,_impl->st[c]>>>(_impl->d_in[c],_impl->d_c[c],n);
    }
    ...
    for(int l=0;l<NUM_DWT_LEVELS;++l){
        ...
        for(int c=0;c<3;++c)
            v11_dwt_h<<<h,BLK,w*sizeof(float),_impl->st[c]>>>(A[c],B_[c],w,h,stride);
        for(int c=0;c<3;++c){float*t=A[c];A[c]=B_[c];B_[c]=t;}

        /* Vertical DWT for all 3 components */
        for(int c=0;c<3;++c)
            v11_dwt_v<<<(w+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A[c],B_[c],w,h,stride);
        for(int c=0;c<3;++c){float*t=A[c];A[c]=B_[c];B_[c]=t;}
    ...
    for(int c=0;c<3;++c){
        v11_qe<<<(tpc+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A[c],_impl->d_enc[c],n,tpc,steps[c]);
