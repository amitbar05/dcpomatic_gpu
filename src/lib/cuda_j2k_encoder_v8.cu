/*
    CUDA JPEG2000 Encoder v8

    Key optimization: Fully parallel shared-memory horizontal DWT.
    All 128 threads participate in every lifting step.
    Each thread processes ~16 samples per step (2048/128=16).

    Lifting steps update odd or even samples independently,
    so multiple threads can work on different samples in parallel.
*/
...
__global__ void v8_i2f(const int32_t*__restrict__ in,float*__restrict__ out,int n){
    int i=blockIdx.x*blockDim.x+threadIdx.x; if(i<n) out[i]=__int2float_rn(in[i]);
}

/**
 * Fully parallel shared-memory horizontal DWT 9/7.
 * One block per row, all threads participate in each lifting step.
 *
 * For a 2048-wide row with 128 threads:
 * - ~1024 odd samples / 128 threads = 8 samples per thread per step
 * - 4 steps + deinterleave = 5 parallel phases
 */
__global__ void v8_dwt_h_par(float*__restrict__ data, float*__restrict__ out,
                              int w, int h, int s)
{
    ...
    /* Cooperative load */
    for (int i = tid; i < w; i += nt) sm[i] = data[y*s+i];
    __syncthreads();

    /* Step 1: alpha on odd samples (indices 1,3,5,...) */
    for (int x = 1 + tid*2; x < w-1; x += nt*2)
        sm[x] += ALPHA * (sm[x-1] + sm[x+1]);
    /* Boundary: last odd if w is even */
    if (tid == 0 && w > 1 && !(w&1))
        sm[w-1] += 2.f * ALPHA * sm[w-2];
    __syncthreads();

    /* Step 2: beta on even samples (indices 0,2,4,...) */
    if (tid == 0)
        sm[0] += 2.f * BETA * sm[min(1,w-1)];
    for (int x = 2 + tid*2; x < w; x += nt*2)
        sm[x] += BETA * (sm[x-1] + sm[min(x+1,w-1)]);
    __syncthreads();

    /* Step 3: gamma on odd samples */
    for (int x = 1 + tid*2; x < w-1; x += nt*2)
...
__global__ void v8_dwt_v(float*__restrict__ data,float*__restrict__ out,int w,int h,int s){
...
__global__ void v8_qe(const float*__restrict__ src,uint8_t*__restrict__ out,int n,int tn,float step){
...
struct CudaJ2KEncoderImpl {
    ...
    void ensure(int w,int h,size_t epc){
        ...
        char*hp=h_pool;for(int c=0;c<3;++c){h_in[c]=(int32_t*)hp;hp+=n*4;}h_enc=(uint8_t*)hp;px=n;
    }
                        ...
                        int64_t bit_rate,int fps,bool is_3d,bool is_4k)
{
    ...
    for(int c=0;c<3;++c){
        ...
        v8_i2f<<<G,BLK,0,_impl->st[c]>>>(_impl->d_in[c],_impl->d_c[c],n);
        int w=width,h=height;float*A=_impl->d_c[c],*B_=_impl->d_t[c];
        for(int l=0;l<NUM_DWT_LEVELS;++l){
            v8_dwt_h_par<<<h,BLK,w*sizeof(float),_impl->st[c]>>>(A,B_,w,h,stride);
            float*t=A;A=B_;B_=t;
            v8_dwt_v<<<(w+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A,B_,w,h,stride);
            t=A;A=B_;B_=t;
            w=(w+1)/2;h=(h+1)/2;
        }
        v8_qe<<<(tpc+BLK-1)/BLK,BLK,0,_impl->st[c]>>>(A,_impl->d_enc[c],n,tpc,steps[c]);
    ...
    cs.reserve(_impl->hdr.size()+20+td+2);cs=_impl->hdr;
